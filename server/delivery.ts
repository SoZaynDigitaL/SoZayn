...
async function submitToUberDirect(order: any) {
  const API_KEY = process.env.UBER_DIRECT_API_KEY;
  const API_URL = process.env.UBER_DIRECT_API_URL;

  if (!API_KEY || !API_URL) {
    throw new Error("UberDirect API credentials are missing in the environment.");
  }
...
async function submitToJetGo(order: any) {
  const API_KEY = process.env.JETGO_API_KEY;
  const API_URL = process.env.JETGO_API_URL;

  if (!API_KEY || !API_URL) {
    throw new Error("JetGo API credentials are missing in the environment.");
  }
